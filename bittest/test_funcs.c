...
#define CHECK_PERM_INV(N, func, ifunc)                                                             \
    uint##N##_t expect, actual, invert;                                                            \
    expect = expect_##func##N(test_case);                                                          \
    check##N##_perm_assert(#func, test_case, expect);                                              \
    actual = func##N(test_case);                                                                   \
    check##N##_impl(#func, test_case, expect, actual);                                             \
    ...
    invert = ifunc##N(actual);                                                                     \
    check##N##_impl(#ifunc, actual, expect, invert);                                               \
    check##N##_inv(#func, #ifunc, test_case, invert);
...
#define CHECK_MANI_INV(N, func, ifunc)                                                             \
    ...
    actual = func##N(test_case);                                                                   \
    check##N##_impl(#func, test_case, expect, actual);                                             \
    expect = expect_##ifunc##N(test_case);                                                         \
    invert = ifunc##N(actual);                                                                     \
    check##N##_impl(#func, actual, expect, invert);                                                \
    check##N##_inv(#func, #ifunc, test_case, invert);
...
#define CALC_TEST(N, func)                                                                         \
    CHECK_CALC(N, func)
#define TEST_CALC(N, func)                                                                         \
    TEST_ALL_CASES(N, func, CALC_TEST)

#define CALC_PAIR_TEST(N, func)                                                                    \
    FOREACH_PAIR_CASE(N)                                                                           \
        CHECK_CALC_PAIR(N, func)                                                                   \
    END_FOREACH
#define TEST_CALC_PAIR(N, func)                                                                    \
    TEST_ALL_CASES(N, func, CALC_PAIR_TEST)

#define INV_TEST(N, func, ifunc)                                                                   \
    CHECK_INV(N, func, ifunc)
#define TEST_INV(N, func, ifunc)                                                                   \
    TEST_ALL_CASES_INV(N, func, ifunc, INV_TEST)

#define PERM_TEST(N, func)                                                                         \
    CHECK_PERM(N, func)
#define TEST_PERM(N, func)                                                                         \
    TEST_ALL_CASES(N, func, PERM_TEST)

#define PERM_INV_TEST(N, func, ifunc)                                                              \
    CHECK_PERM_INV(N, func, ifunc)
#define TEST_PERM_INV(N, func, ifunc)                                                              \
    TEST_ALL_CASES_INV(N, func, ifunc, PERM_INV_TEST)

#define PERM_MASK_INV_TEST(N, func, ifunc)                                                         \
    FOREACH_TEST_MASK(N)                                                                           \
        CHECK_PERM_MASK_INV(N, func, ifunc)                                                        \
    END_FOREACH
#define TEST_PERM_MASK_INV(N, func, ifunc)                                                         \
    TEST_ALL_CASES_INV(N, func, ifunc, PERM_MASK_INV_TEST)

#define MANI_MASK_TEST(N, func)                                                                    \
    FOREACH_TEST_MASK(N)                                                                           \
        CHECK_MANI_MASK(N, func)                                                                   \
    END_FOREACH
#define TEST_MANI_MASK(N, func)                                                                    \
    TEST_ALL_CASES(N, func, MANI_MASK_TEST)
...
#define TEST_MANI_MASK_INV(func, ifunc)                                                            \
    TEST_ALL_CASES_INV(N, func, ifunc, MANI_MASK_INV_TEST)
...
#define OMFLIP_PERM_TEST(N, func)                                                                  \
    ...
    uint##N##_t mirror_mask, invert, actual, expect;                                               \
    FOREACH_TEST_MASK(N)                                                                           \
